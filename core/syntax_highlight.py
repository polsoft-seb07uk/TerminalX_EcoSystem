...
class SyntaxHighlighter:
    ...
    def __init__(self):
        ...
        self.highlight_paths     = True
        self.highlight_numbers   = True
        self.highlight_strings   = True
        self.highlight_comments  = True
        self.highlight_pipes     = True

    def apply_config(self, cfg_get) -> None:
        """Wczytaj ustawienia z config.get(). Wywołane przy setup i po każdym config set."""
        self.enabled             = bool(cfg_get("syntax.enabled",             True))
        theme = str(cfg_get("syntax.theme", "default"))
        if theme in _THEMES:
            self.theme = theme
        self.highlight_paths     = bool(cfg_get("syntax.highlight_paths",    True))
        self.highlight_numbers   = bool(cfg_get("syntax.highlight_numbers",  True))
        self.highlight_strings   = bool(cfg_get("syntax.highlight_strings",  True))
        self.highlight_comments  = bool(cfg_get("syntax.highlight_comments", True))
        self.highlight_pipes     = bool(cfg_get("syntax.highlight_pipes",    True))
    ...
    def colorize(self, line: str) -> str:
        ...
        if not self.enabled or not _ansi_ok() or not line.strip():
            return line
        # Przekaż flagi do metod kolorowania przez kontekst self
        ...
        if len(segments) > 1:
            sep = c["pipe"](" | ") if self.highlight_pipes else " | "
            result = sep.join(colored_segs)
        else:
            result = colored_segs[0]
        return result + RST
    ...
    def _colorize_args(self, tokens: list[str], c: dict) -> str:
        ...
        while i < len(tokens):
            ...
            if self.highlight_comments and tok.startswith("#"):
                rest = " ".join(tokens[i:])
                result.append(c["comment"](rest))
                break

            # String (cudzyslow)
            if self.highlight_strings and tok and tok[0] in ('"', "'"):
            ...
            # Sciezka
            elif self.highlight_paths and _RE_PATH.fullmatch(tok):
                result.append(c["path"](tok))

            # Liczba
            elif self.highlight_numbers and _RE_NUMBER.fullmatch(tok):
...
def setup(terminal) -> None:
    ...
    # Wczytaj ustawienia z config (jesli zaladowany)
    try:
        from . import _integration as _intg
        cfg = _intg.get("config")
        if cfg and callable(cfg.get("get")):
            _highlighter.apply_config(cfg["get"])
    except Exception:
        pass
    ...
    try:
        ...
        _intg.register("syntax_highlight", {
            ...
            "apply_config":   _highlighter.apply_config,
            "set_theme":      _highlighter.set_theme,
